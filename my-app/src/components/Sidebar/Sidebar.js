...
function Sidebar({ userName: propUserName, role: propRole, activePageId, onMenuItemClick, onLogout }) {
    const [username, setUsername] = useState(propUserName || 'Đang tải...');
    const [role, setRole] = useState(propRole || 'user');

    useEffect(() => {
        setUsername(propUserName || 'Người dùng ẩn danh');
        setRole(propRole || 'user');
    }, [propUserName, propRole]);
